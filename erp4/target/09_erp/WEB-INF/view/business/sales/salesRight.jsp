...
<body class="childrenBody">
		...
		<!--操作-->
		<script type="text/html" id="salesListBar">
...
layui.use(['form','layer','table','laydate'],function(){
    ...
    //商品列表
     tableIns = table.render({
        elem: '#salesList',
        url : '${ctx }/sales/loadSales.action',
        cellMinWidth : 95,
        page : true,
        height : "full-200",
        limits : [10,15,20,25],
        ...
        cols : [[
            ...
            {field: 'salestime', title: '销售时间', minWidth:100, align:"center"},
            {field: 'number', title: '销售数量数据', minWidth:100, align:"center"},
            {field: 'salesprice', title: '销售价格', minWidth:100, align:"center"},
            {title: '操作', minWidth:175, templet:'#salesListBar',fixed:"right",align:"center"}
        ]]
    });
  ...
  //监听头工具栏事件
    ...
    $(".search_btn").on("click",function(){
    	...
    	table.reload('salesListTable', {
    		  url: '${ctx }/sales/loadSales.action?'+params
    	});
    });
   ...
   function toAddSales(){
        var index = layui.layer.open({
            ...
            type : 2,//ifream层
            area:["700px","500px"],
            content : "${ctx }/sales/toAddSales.action",
    ...
    function updateSales(data){
    	var index = layui.layer.open({
            ...
            type : 2,
            area:["700px","500px"],
            content : "${ctx }/sales/toUpdateSales.action?id="+data.id,
            success : function(layero, index){
                setTimeout(function(){
                    layui.layer.tips('点击此处返回商品列表', '.layui-layer-setwin .layui-layer-close', {
                        tips: 3
    ...
    table.on('tool(salesList)', function(obj){
        var layEvent = obj.event,
            data = obj.data;
        if(layEvent === 'edit'){ //编辑
        	updateSales(data);//data主当前点击的行
        }else if(layEvent === 'del'){ //删除
            layer.confirm('确定删【'+data.salesname+'】商品吗？',{icon:3, title:'提示信息'},function(index){
                 $.post("${ctx }/sales/deleteSales.action",{
  ...
  //批量删除
    function batchDelete(){
        ...
        var checkStatus = table.checkStatus('salesListTable'),//选中状态
        ...
        if(data.length > 0) {
            ...
            layer.confirm('确定删除选中的商品？', {icon: 3, title: '提示信息'}, function (index) {
                 $.post("${ctx }/sales/batchDeleteSales.action?"+ids,function(data){
                	 //刷新table
                     tableIns.reload();
                 	 //关闭提示框
                	layer.close(index);
